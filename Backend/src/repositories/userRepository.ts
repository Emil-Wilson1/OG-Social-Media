...
export class UserRepository {
  ...
  async createUser(
    ...
    const newUser = await this.tempUserModel.create({
      fullname,
      email,
      username,
      password,
  ...
  async deleteOTP(email: string): Promise<void> {
    await this.tempUserModel
      .findOneAndUpdate({ email }, { $unset: { otp: 1 } }, { new: true })
      .exec();
  }
  async getUser(email: string): Promise<tempUserDocument | null> {
    return await this.tempUserModel.findOne(
      { email },
      { otp: 1, createdAt: 1 } 
    ).exec();
  }
  ...
  async updatePrivacy(
    ...
    return this.userModel
      .findByIdAndUpdate(userId, { isPrivate }, { new: true })
      .exec();
  }
