...
const protect = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (authHeader && authHeader.startsWith('Bearer ')) {
    ...
    try {
      ...
      const user = await authService.verifyUser(decoded);
...
const refreshAccessToken = async (req: Request, res: Response) => {
  try {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      ...
      if (refreshToken) {
        console.log('Received refresh token:', refreshToken);
        const decodedRefreshToken: any = authService.verifyToken(refreshToken);
        console.log('Decoded refresh token:', decodedRefreshToken);

        const user = await authService.verifyUser(decodedRefreshToken);
        if (user && !user.blocked) {
          const newAccessToken = await authService.renewAccessToken(decodedRefreshToken.userId);
          console.log('Generated new access token:', newAccessToken);
          res.status(HttpStatusCode.OK).send({ newToken: newAccessToken });
        } else {
          console.warn('User not authorized or blocked');
          res.status(HttpStatusCode.UNAUTHORIZED).json({
            message: 'User not authorized',
...
export { protect, refreshAccessToken };
