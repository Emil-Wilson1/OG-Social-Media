...
class AuthService {
  constructor(
    ...
    private mailer: { sendOTP: (email: string, otp: number) => Promise<void> }
  ) {}
  ...
  async signup(
    ...
    try {
      ...
      cron.schedule("*/1 * * * * *", async () => {
        ...
        if (user?.otp && this.isOTPExpired(user.createdAt)) {
          try {
  ...
  > {
    ...
    const accessToken = jwt.sign({ userId: user._id }, "your_secret_key_here", {
      expiresIn: "1hr",
    });
  ...
  async sendPasswordResetEmail(email: string): Promise<string> {
    try {
      ...
      const resetLink = `https://socioclub.site/forgot?token=${resetToken}`;
